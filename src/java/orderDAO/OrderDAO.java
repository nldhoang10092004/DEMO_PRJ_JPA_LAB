...
public class OrderDAO implements IOrderDAO {
    ...
    public void insertOrder(Order orderObj) throws SQLException {
        try (Connection con = DBConnection.getConnection(); PreparedStatement ps = con.prepareStatement(INSERT_ORDER)) {
    ...
    public Order getOrderById(int id) {
        Order order = null;
        try (Connection con = DBConnection.getConnection(); PreparedStatement ps = con.prepareStatement(SELECT_ORDER_BY_ID)) {
            ps.setInt(1, id);
    ...
    public int createOrder(Order order) {
        int generatedId = -1;
        try (Connection con = DBConnection.getConnection(); // ✅ Bổ sung RETURN_GENERATED_KEYS
    ...
    public void addOrderDetail(int orderId, int productId, int quantity, Double price) {
        try (Connection con = DBConnection.getConnection(); PreparedStatement ps = con.prepareStatement(INSERT_ORDERDetails)) {
            ps.setInt(1, orderId);
